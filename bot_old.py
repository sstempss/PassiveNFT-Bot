...
sub_data = self.config.SUBSCRIPTIONS[sub_type]
message = f"**{sub_data['name']}**\n\n"
message += f"{sub_data['nft_per_day']} NFT в день, {sub_data['gifts_per_day']} гифта в день\n"
message += f"{sub_data['nft_per_month']} NFT в месяц, {sub_data['gifts_per_month']} гифтов в месяц\n\n"
message += f"Процент победы одного участника составляет {sub_data['nft_win_percentage']}% на одно NFT, "
message += f"количество разыгрываемых NFT в день – {sub_data['nft_per_day']}, "
message += f"следственно {sub_data['nft_per_day']}*{sub_data['nft_win_percentage']}% = {sub_data['nft_daily_wins']}% на победу за день, "
message += f"в месяц получается {sub_data['nft_monthly_wins']}%\n\n"
if 'gifts_win_percentage' in sub_data:
message += f"На гифты за звезды процент победы на одного участника составляет {sub_data['gifts_win_percentage']}%, "
message += f"количество разыгрываемых гифтов в день – {sub_data['gifts_per_day']}, "
message += f"следственно {sub_data['gifts_per_day']}*{sub_data['gifts_win_percentage']}% = {sub_data['gifts_daily_wins']}% на победу за день, "
message += f"в месяц получается {sub_data['gifts_monthly_wins']}%\n\n"
if 'min_refund' in sub_data:
message += f"{sub_data['min_refund']}\n\n"
elif 'refund' in sub_data:
message += f"{sub_data['refund']}\n\n"
message += f"~ окуп от {sub_data['roi_range']}\n"
keyboard = [
...
"""
keyboard = [
[InlineKeyboardButton("Назад", callback_data="back_to_subscriptions")]
]
reply_markup = InlineKeyboardMarkup(keyboard)
await query.edit_message_text(
payment_text,
