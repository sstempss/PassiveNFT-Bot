...
# -*- coding: utf-8 -*-
"""
ФИНАЛЬНАЯ КОНФИГУРАЦИЯ для Render.com deployment
Вставляется в config_deploy_new.py в GitHub
"""
...
class SafeConfig:
    """Конфигурация бота - ФИНАЛЬНАЯ ВЕРСИЯ с полным функционалом"""
...
🔗 Она состоит из пригласительной ссылки, где владелец ссылки получается 10% с его оплаты подписки, для более точных подробностей свяжитесь с [менеджером](https://t.me/{self.MANAGER_USERNAME})"""
        ...
        self.REFERRAL_WELCOME_MESSAGE = f"""🎉 welcome to the PassiveNFT 🎉
...
чтобы ознакомиться с тем что входит в подписку, выберите заинтересовавший вас вариант снизу"""
        ...
        # Шаблон для оплаты через звездочки  
...
⚠️ ВАЖНО: Скопируйте адрес кошелька и отправьте указанную сумму TON."""
        ...
        self.ADMIN_PANEL_MESSAGE = """🎛️ Админ панель PassiveNFT:
👥 Рефералы: статистика и управление
💰 Заработок: отслеживание комиссий
📊 Статистика: данные по пользователям
⚙️ Настройки: управление ботом"""

        self.MANAGER_CONTACT_MESSAGE = f"""👤 Связь с менеджером
Для связи с менеджером проекта напишите [здесь](https://t.me/{self.MANAGER_USERNAME})
❓ Здесь вы можете получить ответы на любые вопросы о проекте, подписках и оплате"""
...
logging.info("✅ Финальная конфигурация загружена из config_deploy_new.py")
